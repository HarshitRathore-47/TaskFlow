...
export default function TasksSummary() {
  ...
  const myTasks = useMemo(() => tasks.filter((i) => i.assigneeId === user?.id), [tasks, user?.id]);
  ...
  const inProgressIssues = useMemo(() => tasks.filter((i) => i.status === "IN_PROGRESS"), [tasks]);
  ...
  return (
    <div className="space-y-6">
      {summaryCards.map((card) => (
        <div
          key={card.title}
          className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-200 rounded-lg overflow-hidden shadow-sm"
        >
